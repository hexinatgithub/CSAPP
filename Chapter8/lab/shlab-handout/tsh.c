...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(argv))
    {
        if ((pid = fork()) == 0)
        {
            ...
            if ((setpgid(pid, pid) == -1) || (execve(argv[0], argv, environ) < 0))
            {
        ...
        addjob(&jobs[nextjid-1], pid, state, buf);
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WUNTRACED | WNOHANG)) > 0)
    {
        ...
        if (WIFSIGNALED(status)){
            printf("Child %d terminated abnormally with signal :%d \n", pid, WTERMSIG(status));
            deletejob(jobs, pid);
        }
        if (WIFSTOPPED(status))
        {
...
void sigint_handler(int sig) 
{
    ...
    if (pid != 0){
        kill(-pid, sig);
        struct job_t *job = getjobpid(jobs, pid);
        printf("Job [%d] interput by signal %d\n", job->jid, sig);
        deletejob(jobs, pid);
    }
    return;
}
